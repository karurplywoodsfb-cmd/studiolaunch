...
const TABS: { value: Tab; label: string }[] = [
  ...
  { value: 'contact',    label: 'Contact & Branding' },
  { value: 'seo',        label: 'SEO Enrichment' },
...
function SettingsInner() {
  ...
  return (
    <div className="max-w-3xl">
      ...
      {/* Tabs */}
      <div className="flex gap-1 mb-8 overflow-x-auto pb-1 border-b border-[#1A1A1A]">
        {TABS.map(t => (
          <button key={t.value} onClick={() => setTab(t.value)}
            className={`text-xs px-4 py-3 whitespace-nowrap transition-colors border-b-2 -mb-px ${
              tab === t.value ? 'text-[#C8A96E] border-[#C8A96E]' : 'text-[#6B6B6B] border-transparent hover:text-[#F5F0E8]'
            }`}>{t.label}</button>
        ))}
      </div>
      ...
      {showSave && (
        <div className="mt-8 flex items-center gap-4 pt-6 border-t border-[#1A1A1A]">
          <button onClick={handleSave} disabled={saving}
            className="bg-[#C8A96E] text-[#0A0A0A] text-xs font-semibold tracking-widest uppercase px-8 py-3 hover:bg-[#A8854A] transition-colors disabled:opacity-50">
